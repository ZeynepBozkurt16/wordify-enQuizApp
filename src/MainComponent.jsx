  ...
  return (
    <div className='background'>
      ...
      <div className='container-main'>
        ...
        {!loading && <button className='activate buttonAdd' onClick={addLine}>EKLE</button>}
        {!loading && <button className='activate buttonRemove' onClick={saveItems}>KAYDET</button>}
        {role === 'admin' && <button className='activate buttonRemove' onClick={adminDash}>YÖNETİCİ</button>}
        {!loading && <button className='activate buttonRemove' onClick={signOut}>ÇIKIŞ YAP</button>}
        {inputData.length > 0 && <button className='activate buttonFinish' onClick={navigateToFinal}>Bitir</button>}
        {!loading && <button className='activate buttonRemove' onClick={navigateTest}>Test Et</button>}
      </div>
    </div>
  );
};
