...
const Final = () => {
  ...
  const currentImage = inputData[number]?.image; // Resim bilgisi
  const currentSentence = inputData[number]?.sentence; // Cümle
  ...
  const speakText = (text) => {
    if ('speechSynthesis' in window) {
      ...
      speech.lang = 'en-US'; // Dili ayarlayabilirsiniz
      speech.onend = () => {
        setShowAnswer(true);
      };
        ...
        <div className='quizCard' onClick={reverseState}>
          ...
          {showAnswer && !editState && (
            <div className="back">
              ...
              <p>{currentAnswer}</p>
              <p>{currentSentence}</p> {/* Cümle ekleyin */}
